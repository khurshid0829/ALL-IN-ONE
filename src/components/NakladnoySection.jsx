...
function makeEmptyLine() {
  return { key: makeLineKey(), selectedSku: null, qty: '' }
}
  ...
  function addLinesFromMultiSelect(items) {
    setLines((prev) => {
      ...
      const newLines = items
        ...
        .map((item) => ({ key: makeLineKey(), selectedSku: item, qty: '' }))
      if (newLines.length === 0) return prev

      const isSingleEmptyLine = prev.length === 1 && !prev[0].selectedSku && !prev[0].qty
